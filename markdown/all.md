...
> Generated at: 2026/9/6 18:56:40
> File count: 194
...
`euv-engine` 的光照模块提供纯函数形式的光照计算（无 GPU 着色器依赖），既可被 WebGPU/WebGL 着色器在 GPU 上并行执行，也可被 CPU 软件渲染器串行调用。
...
支持三类光源（方向光 / 点光 / 聚光）、三类材质（Lambert / Phong / PBR 占位），以及 Phong 反射、距离衰减、软阴影射线相交。
...
`shade` 是核心入口。它会遍历每个光源，结合 `soft_shadow_factor` 计算可见性，对点光自动应用 `apply_falloff(view_distance, falloff)`，再叠加漫反射与 Phong 镜面。
...
- 上游：被 `WebGlRenderer` / `WebGpuRenderer` 在着色器中调用同名数学（在 Rust 侧预计算 uniform），也被 CPU 软件渲染器在每像素循环中调用。
- 下游：与 [`raytracing`](./raytracing.md) 模块配合，由 `LightingUniforms::shade` 在每次 `trace_default` 求最近 hit 时调用。
...
`euv-engine` 的光线追踪模块提供与 GPU 无关的纯函数光线求交与递归追踪工具，既能驱动 WebGPU/WebGL 着色器的实时光追前向 pass，也能直接驱动 CPU 软件光线追踪器（教学 / Demo / 离线渲染）。
...
- 上游：被 CPU 软件光线追踪器在每像素循环中调用；也可直接被 `WebGpuRenderer` 的 compute shader 或 ray-tracing pipeline 引用同一套数学。
- 下游：调用 [`lighting`](./lighting.md) 模块的 `LightingUniforms::shade` 来计算每次命中后的着色贡献。

> [!tip]
>
> 本模块仅依赖 `Vector3D` 与 `Material`，完全不依赖任何 GPU / WebGL / WebGPU 资源。CPU 渲染路径与 GPU 着色器共享同一份 Rust 数学实现，避免"GPU 上一个公式、CPU 上另一个公式"导致的视觉漂移。
